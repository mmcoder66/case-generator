...
4. 按章节名检查并读取 `inputs/<project>/ui_design/<章节名>/`。
...
- 新生成或补充的用例只写入 `outputs/<project>/business_site/origin_exports/` 根目录，不再按项目分类创建子目录。
- Excel 交付默认分需求导出：`outputs/<project>/business_site/excel_exports/<module_name>_testcases.xlsx`。
