...
> 这是一个**通用骨架版本**：业务关键字、站点分类、追踪字段、核心链路词表都通过 `scripts/business_constants.py` 配置；业务知识存放在 `knowledge_base/`；新项目接入时只需替换这两个位置的业务内容，框架代码无需修改。
...
│       └── <site_type>/<level1_menu>/<level2_menu>_template.md
├── inputs/               # 输入层：本次生成的素材
│   ├── requirements/     # 需求文档
...
└── outputs/              # 输出层
    ├── origin_exports/   # 原始测试用例（Markdown），按 site_type 分目录
    └── excel_exports/    # Excel 交付文件，按 site_type 分目录
```

> 站点分类（`<site_type>`）由 `scripts/business_constants.py` 的 `SITE_TYPES` 决定，骨架版本默认只有 `default_site` 占位；新项目按业务实际拆分站点后同步修改。
...
**参考用例只读**：`testcase_templates/` 存放高质量范例，AI 只读不写，新生成内容一律写入 `outputs/origin_exports/<site_type>/`，模板不被污染。
...
- `CPV_SPECIFIC_HEADERS`：项目专属追踪字段（拼接到通用 9 列后）
- `SITE_TYPES`：项目站点分类
...
按 `knowledge_base/README.md` 推荐结构，根据项目实际填充：
...
- **scripts/business_constants.py**：业务常量集中入口（站点分类、追踪字段、Excel 品牌、核心链路词表、业务校验开关），换项目时改这一处即可全局生效
...
- **outputs/origin_exports/&lt;site_type&gt;/**：原始 Markdown 用例保存位置
- **outputs/excel_exports/&lt;site_type&gt;/**：Excel 导出文件保存位置

## 输出站点分类

站点分类由 `business_constants.SITE_TYPES` 决定，骨架项目默认只有一个占位值 `default_site`。新项目按业务实际拆分站点：

| 站点分类 | Markdown 输出目录 | Excel 输出目录 | 适用模块 |
|---|---|---|---|
| `default_site` | `outputs/origin_exports/default_site/` | `outputs/excel_exports/default_site/` | <TODO: 按 SITE_TYPES 配置实际站点和适用模块> |

输出文件仍按业务模块聚合，不按二级菜单继续拆分。

## 参考模块路径

参考文件路径以 `testcase_templates/modules/menu_index.md` 为准，根 README 不再重复维护完整索引表。每个 `<site_type>/` 下只维护一级菜单目录；一级菜单下的参考文件统一以 `_template.md` 作为公共后缀。二级菜单未拆分或本身已足够精确时用 `<level2_menu>_template.md`，同一二级菜单下按需求或子功能拆分时用 `<level2_menu>_<requirement_or_submodule>_template.md`。目录名和文件名必须全部小写，不带空格，以 `_` 分隔单词。
