...
换项目时，本文件是要替换的核心入口——所有业务关键字、
表头字段约定、Excel 导出品牌都集中在这里。

换项目检查清单：
    1. PROJECT_SPECIFIC_HEADERS —— 改成新项目的追踪字段（表头尾部列）
    2. SHEET_NAME / CREATOR_NAME —— 改 Excel 品牌
    3. COLUMN_WIDTH_BY_HEADER —— 按新表头字段调整列宽
    4. CORE_FLOW_KEYWORDS —— 替换成新项目的核心业务模块词表（或置空）
    5. ENABLED_BUSINESS_RULES —— 按保留的业务规则开关
"""
import re
...
PROJECT_SPECIFIC_HEADERS: list[str] = []  # TODO: 填入新项目专属字段，如 ["是否自动化", "关联接口", ...]
...
COLUMN_WIDTH_BY_HEADER = {
    ...
    "备注": 24, "用例标签": 18,
    # TODO: 为 PROJECT_SPECIFIC_HEADERS 中每个字段补列宽
}
