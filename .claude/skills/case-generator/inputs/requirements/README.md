...
- UI 设计图不放在本目录，按章节名放在 `inputs/ui_design/<章节名>/` 下。
- 本目录文件不会被脚本自动扫描，只有用户在对话中指定文件和章节时才读取。

示例：

```text
根据 inputs/requirements/raw_docs/<文件名>.docx 的"<章节名>"章节生成测试用例
```
...
提取时图片会被忽略，文字和表格尽量保留为可读文本。UI 设计图不放在需求目录，按章节名放在 `inputs/ui_design/<章节名>/` 下。
